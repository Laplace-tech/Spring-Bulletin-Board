...
//  BaseTime 클래스는 JPA 엔티티에서 공통적으로 사용할 시간 정보(생성일자, 수정일자)를 관리함
	...
	protected void setCreatedDate() {
		log.info("[setCreatedDate] >> createdDate({} -> {}) " , this.createdDate, LocalDateTime.now());
	...
	protected void updateLastModifiedDate() {
		log.info("[updateLastModifiedDate] >> lastModifiedDate({} -> {}) " , this.lastModifiedDate, LocalDateTime.now());
...
//		log.info("BaseTime @Getter getLastModifiedDate called. Current lastModifiedDate : {}", lastModifiedDate);
