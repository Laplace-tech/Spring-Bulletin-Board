...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		 ...
		 */
				...
				.formLogin(formLogin -> formLogin.loginPage("/user/login").defaultSuccessUrl("/").failureHandler(customFailureHandler))
     ...
     * Spring 은 의존성 주입(Dependency Injection)을 통해 필요한 객체들을 자동으로 주입함.
     * @Bean으로 등록된 객체들은 스프링 컨텍스트가 시작될 때, 컨테이너에 빈으로 등록되고 스프링 애플리케이션에서 이를 
     * 사용할 수 있음.
     */
    @Bean
    public BCryptPasswordEncoder passwordEncoder() {
		/* 반환하는 BCryptPasswordEncoder 객체를 생성하여 Spring 의 빈 컨테이너에 등록한다.
		 * 이를 애플리케이션 어디에서나 주입받을 수 있게된다. 즉, 클래스마다 각각 새로운 객체를 생성할 필요 없이
		 * @Bean 으로 선언한 객체를 참조할 수 있는 변수를 선언하기만 하면 그것을 사용할 수 있다는 뜻.
		 */
	...
	AuthenticationManager authenticationManager(AuthenticationConfiguration authenticationConfiguration)
			throws Exception {
		/* AuthenticationManager 는 Spring Security 에서 인증을 담당하는 컴포넌트임.
		 * 이 객체는 ** AuthenticationProvider ** 와 협력하여 사용자가 로그인 폼에 
		 * 작성한 인증 정보를 검증하고 인증 여부를 판단함.
		 * 
		 * AuthenticationManager 는 AuthenticationProvider 인터페이스를 구현한 클래스들에 의해
		 * 인증을 처리한다. 우선, AuthenticationManager 가 호출되며느 여러 AuthenticationProvider 중 하나가
		 * 인증 절차를 담당하고, 그 과정에서 ** loadUserByUsername() 과 passwordEncoder를 호출함 ** 
		 */
