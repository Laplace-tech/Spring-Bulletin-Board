...
public class ArticleViewController {
	...
	public String viewArticlesByPage(@RequestParam(value = "page", defaultValue = "0") int pageNum, Model model) {
		...
		log.info("Fetching articles for page number : {}", pageNum);
		
		Page<ArticleViewDto> articlePage = this.articleService.readArticleViewPage(pageNum);
		model.addAttribute("articlePage", articlePage);
		
		// 로깅 : 페이지 조회 후 결과 출력
		log.info("Fetched {} articles for page {}", articlePage.getContent().size(), pageNum);
	...
	public String viewArticleDetail(@PathVariable("id") Long articleID,
		...
		log.info("Fetching details for article with ID : {}", articleID);
		...
		// 로깅 : 
		log.info("Fetched article details for ID : {} - Title : {}", article.getId(), article.getTitle());
...
//		=> 이 방식이 ㅈ같으면 Model 로 입히던가	
	...
	public String showUpdateArticleForm(@PathVariable("id") Long articleID, CommentSubmitForm commentSubmitForm,
