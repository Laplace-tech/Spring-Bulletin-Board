...
public class ArticleViewController {
	...
	public String viewArticlesByPage(@RequestParam(value = "page", defaultValue = "0") int pageNum, Model model) {
		...
		// 로깅 : 페이지 조회 후 결과 출력
		log.info("[GET: /articles?page={}] Retrieved {} articles page {}", pageNum, articlePage.getSize(), pageNum);
		return "article_list";
	}
	...
	public String viewArticleDetail(@PathVariable("id") Long articleID,
		...
		log.info("[GET: /articles/detail/{}] Request received.", articleID);
	...
	public String viewCreateArticleForm(@ModelAttribute ArticleSubmitForm articleSubmitForm, Principal principal) {
		...
		log.info("[GET: /articles/create] Request received by user : {}", principal.getName());
		 ...
		 * 따라서, articleSubmitForm 이라는 변수명으로 유저 뷰에 전달이 되는데 이때, 
	...
	public String viewUpdateArticleForm(@PathVariable("id") Long articleID,
		...
		log.info("[GET: /articles/modify/{}] Request received by user : {}", articleID, principal.getName());
		
		// 권한 체크
		log.info("[GET: /articles/modify/{}] Checking update permissions for user : {}", articleID, principal.getName());
		if (!this.articleService.hasAuthorPermission(articleID, principal)) {
		...
		log.info("[GET: /articles/modify/{}] Update form loaded successfully for user : {}", principal.getName());
		return "article_form"; // 수정 폼 페이지로 이동
	}

	/*
	 * @PreAuthorize("isAuthenticated()") in Spring Security
	 * 인증된 사용자만 해당 메서드를 호출할 수 있게 함. 즉, 로그인 하지 않았다면 이 메서드를 호출할 수 없음.
	 * 인증되지 않은 사용자는 403 Forbidden 에러를 받는다. (접근권한 없음)
	 */
	@PreAuthorize("isAuthenticated()")
	...
	public String createArticleForm(@Valid @ModelAttribute ArticleSubmitForm articleSubmitForm,
		...
		log.info("[POST: /articles/create] Request received from user : {}", principal.getName());
	...
	public String updateArticleForm(@Valid ArticleSubmitForm articleSubmitForm, @PathVariable("id") Long articleID,
		...
		log.info("[PATCH: /articles/modify/{}] Request received by user : {}", articleID, principal.getName());
		
		// 권한 확인
		log.info("[PATCH: /articles/modify/{}] Checking update permission for user : {}", articleID, principal.getName());
		if (!articleService.hasAuthorPermission(articleID, principal)) {
		...
		log.info("[PATCH: /articles/modify/{}] Successfully updated article! User : {}, LastModified : {}",
				principal.getName(), modifiedArticle.getLastModifiedDate());
